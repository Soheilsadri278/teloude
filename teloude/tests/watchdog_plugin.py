...
2. Supervisor process, one per session (`_SUPERVISOR_SOURCE`). A separate
   ...
   running, kills pytest (`taskkill /F` on Windows, with bounded `/F /T` tree
   retries, and SIGABRT then SIGKILL elsewhere) and exits with a non-zero status.
   The kill is verified rather than assumed: a terminated process is not called
   alive just because its Windows process object is still openable - a handle
   says nothing, and the object stays referenced until the parent reaps it, which
   cannot happen before this supervisor closes the stdout it inherited -
   `taskkill`'s own exit status reaches the report, and a verified kill, a
   process that was already gone and one that could not be ended are three
   different verdicts. The heartbeat names the session
...
_SUPERVISOR_SOURCE = "\n".join([
    ...
    '            # A handle is NOT proof of life. A terminated process stays',
    '            # openable while anything still holds a handle to its object -',
    '            # and when the supervisor kills pytest, the parent cannot reap it',
    '            # until this process closes the stdout it inherited, so the object',
    '            # is always still referenced here. OpenProcess happily returns a',
    '            # handle for such a pid, which is what made the watchdog announce',
    '            # a successful kill as "could not be killed". The wait state is',
    '            # what separates the two: an exited process signals its object, so',
    '            # a zero-timeout wait returns WAIT_OBJECT_0 (0), not',
    '            # WAIT_TIMEOUT (258).',
    '            handle = kernel32.OpenProcess(0x00100000 | 0x1000, False, pid)',
    '            if not handle:',
    '                # 87 = ERROR_INVALID_PARAMETER: no such process at all.',
    '                # Anything else (access denied) means there is something to',
    '                # deny, so keep treating it as alive and never claim a kill.',
    ...
    '',
    '',
    '#: The three outcomes of a kill attempt. A boolean cannot tell "already',
    '#: gone" from "still running", and a Windows run showed what that costs: a',
    '#: process that had really been terminated was reported as a stuck job.',
    "KILLED = 'killed'",
    ...
    '',
    'def taskkill(extra=()):',
    '    # One bounded kill attempt, with the diagnostics that make a real failure',
    "    # diagnosable: taskkill's exit status and the first line it printed, both",
    '    # of which used to be thrown away (DEVNULL and a bare except).',
    "    command = ['taskkill', '/F', *extra, '/PID', str(parent_pid)]",
    '    try:',
    '        done = subprocess.run(command, stdout=subprocess.PIPE,',
    '                              stderr=subprocess.PIPE, timeout=20)',
    "        detail = (done.stderr or done.stdout or b'').decode('utf-8', 'replace')",
    '        detail = detail.strip().splitlines()',
    "        return done.returncode, (detail[0] if detail else '')",
    '    except Exception as exc:',
    '        return None, repr(exc)',
    '',
    '',
    'def kill_pytest():',
    '    # Three outcomes, never fewer: the process is gone and this process watched',
    '    # it go (KILLED), it was already gone (ALREADY_GONE), or it is still there',
    '    # and could not be ended (NOT_KILLED). Anything less either claims a kill',
    '    # that never landed or calls a dead process a stuck job.',
    '    if not alive(parent_pid):',
    "        report('[watchdog] pytest process is gone; nothing to kill')",
    '        return ALREADY_GONE',
    "    if os.name == 'nt':",
    '        # Escalation, and it is bounded: one pid-targeted kill, then tree',
    '        # retries, each followed by a bounded wait for the pid to disappear.',
    "        for extra in ((), ('/T',), ('/T',)):",
    '            status, detail = taskkill(extra)',
    "            report('[watchdog] taskkill %s -> %s %s'",
    "                   % (('/F ' + ' '.join(extra)).strip(), status,",
    "                      ('(%s)' % detail) if detail else ''))",
    '            if wait_gone(parent_pid, 3.0):',
    '                return KILLED',
    '        return NOT_KILLED',
    ...
    '    return KILLED if wait_gone(parent_pid, 3.0) else NOT_KILLED',
    '',
    '',
    ...
    '            outcome = kill_pytest()',
    '            if outcome == KILLED:',
    "                report('[watchdog] pytest killed and confirmed gone;'",
    "                       ' the job must fail')",
    '            elif outcome == ALREADY_GONE:',
    "                report('[watchdog] pytest was already gone; the job fails with it')",
    '            else:',
    "                report('[watchdog] pytest could not be killed; the job is stuck')",
    "            if os.name == 'nt':",
